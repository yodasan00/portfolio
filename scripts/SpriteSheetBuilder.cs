...
namespace Portfolio
{
    ...
    {
        // Check if color is background (near white / cream background)
        private static bool IsBg(Color c, Color bg)
        {
            ...
            int dg = c.G - bg.G;
            int db = c.B - bg.B;
            return Math.Sqrt(dr * dr + dg * dg + db * db) < 35.0 || (c.R > 240 && c.G > 240 && c.B > 240);
        }
        ...
        public static Rectangle FindBounds(Bitmap bmp, int startX, int endX)
        {
        ...
        public static void BuildSheet(string inputPath, string outputPath, int targetFrameW = 105, int targetFrameH = 115)
        {
            using (Bitmap transparent = RemoveBackground(inputPath))
            {
                ...
                // 4 frame regions along horizontal axis
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    Console.WriteLine("Frame " + i + ": " + bounds[i]);
                }

                // Output 4-frame sheet (420 x 115)
                ...
                using (Graphics g = Graphics.FromImage(sheet))
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        {
                            // Scale factor to fit inside frame comfortably
                    ...
                    Console.WriteLine("Saved compiled sheet to: " + outputPath);
                }
            }
        }
    }
