using System;
using System.Drawing;
using System.Drawing.Imaging;
using System.IO;
using System.Collections.Generic;

namespace Portfolio
{
    public class MenuHoverSheetCompiler
    {
        private static bool IsBg(Color c, Color bg)
        {
            if (c.A < 20) return true;
            int dr = c.R - bg.R;
            int dg = c.G - bg.G;
            int db = c.B - bg.B;
            return Math.Sqrt(dr * dr + dg * dg + db * db) < 35.0 || (c.R > 245 && c.G > 245 && c.B > 245);
        }

        public static Bitmap RemoveBackground(string filePath)
        {
            using (Bitmap src = new Bitmap(filePath))
            {
                int w = src.Width;
                int h = src.Height;
                Bitmap outBmp = new Bitmap(w, h, PixelFormat.Format32bppArgb);

                for (int y = 0; y < h; y++)
                {
                    for (int x = 0; x < w; x++)
                    {
                        outBmp.SetPixel(x, y, src.GetPixel(x, y));
                    }
                }

                Color bg = src.GetPixel(2, 2);
                bool[,] visited = new bool[w, h];
                Queue<Point> q = new Queue<Point>();

                for (int x = 0; x < w; x++)
                {
                    if (IsBg(outBmp.GetPixel(x, 0), bg)) { q.Enqueue(new Point(x, 0)); visited[x, 0] = true; }
                    if (IsBg(outBmp.GetPixel(x, h - 1), bg)) { q.Enqueue(new Point(x, h - 1)); visited[x, h - 1] = true; }
                }
                for (int y = 0; y < h; y++)
                {
                    if (IsBg(outBmp.GetPixel(0, y), bg)) { q.Enqueue(new Point(0, y)); visited[0, y] = true; }
                    if (IsBg(outBmp.GetPixel(w - 1, y), bg)) { q.Enqueue(new Point(w - 1, y)); visited[w - 1, y] = true; }
                }

                while (q.Count > 0)
                {
                    Point p = q.Dequeue();
                    outBmp.SetPixel(p.X, p.Y, Color.FromArgb(0, 0, 0, 0));

                    int[] dx = new int[] { 0, 0, 1, -1 };
                    int[] dy = new int[] { 1, -1, 0, 0 };

                    for (int i = 0; i < 4; i++)
                    {
                        int nx = p.X + dx[i];
                        int ny = p.Y + dy[i];

                        if (nx >= 0 && nx < w && ny >= 0 && ny < h && !visited[nx, ny])
                        {
                            visited[nx, ny] = true;
                            if (IsBg(outBmp.GetPixel(nx, ny), bg))
                            {
                                q.Enqueue(new Point(nx, ny));
                            }
                        }
                    }
                }
                return outBmp;
            }
        }

        public static Rectangle FindBounds(Bitmap bmp, int startX, int endX, int startY, int endY)
        {
            int minX = bmp.Width, maxX = -1, minY = bmp.Height, maxY = -1;
            for (int y = startY; y < endY; y++)
            {
                for (int x = startX; x < endX; x++)
                {
                    if (bmp.GetPixel(x, y).A > 30)
                    {
                        if (x < minX) minX = x;
                        if (x > maxX) maxX = x;
                        if (y < minY) minY = y;
                        if (y > maxY) maxY = y;
                    }
                }
            }
            if (maxX < minX || maxY < minY) return Rectangle.Empty;
            return new Rectangle(minX, minY, maxX - minX + 1, maxY - minY + 1);
        }

        public static void Process(string inputImg, string outputDir)
        {
            using (Bitmap transparent = RemoveBackground(inputImg))
            {
                int w = transparent.Width;
                int h = transparent.Height;
                int midY = h / 2;
                int quarterX = w / 4;

                string[] fileNames = new string[] {
                    "mew2-Sheet.png",
                    "paper2-Sheet.png",
                    "computer2-Sheet.png",
                    "videoGame2-Sheet.png"
                };

                int frameW = 160;
                int frameH = 135;
                int totalW = frameW * 2;
                int totalH = frameH;

                for (int i = 0; i < 4; i++)
                {
                    int startX = i * quarterX;
                    int endX = (i + 1) * quarterX;

                    Rectangle idleBounds = FindBounds(transparent, startX, endX, 0, midY);
                    Rectangle hoverBounds = FindBounds(transparent, startX, endX, midY, h);

                    Console.WriteLine("Icon " + i + " Idle: " + idleBounds + ", Hover: " + hoverBounds);

                    using (Bitmap sheet = new Bitmap(totalW, totalH, PixelFormat.Format32bppArgb))
                    using (Graphics g = Graphics.FromImage(sheet))
                    {
                        g.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.NearestNeighbor;
                        g.PixelOffsetMode = System.Drawing.Drawing2D.PixelOffsetMode.Half;

                        int maxDim = Math.Max(Math.Max(idleBounds.Width, idleBounds.Height), Math.Max(hoverBounds.Width, hoverBounds.Height));
                        double scale = Math.Min((double)(frameH - 12) / maxDim, (double)(frameW - 12) / maxDim);

                        // Draw Idle (Frame 1)
                        if (idleBounds != Rectangle.Empty)
                        {
                            int dw1 = (int)(idleBounds.Width * scale);
                            int dh1 = (int)(idleBounds.Height * scale);
                            int dx1 = (frameW - dw1) / 2;
                            int dy1 = (frameH - dh1) / 2;
                            g.DrawImage(transparent, new Rectangle(dx1, dy1, dw1, dh1), idleBounds, GraphicsUnit.Pixel);
                        }

                        // Draw Hover (Frame 2)
                        if (hoverBounds != Rectangle.Empty)
                        {
                            int dw2 = (int)(hoverBounds.Width * scale);
                            int dh2 = (int)(hoverBounds.Height * scale);
                            int dx2 = frameW + (frameW - dw2) / 2;
                            int dy2 = (frameH - dh2) / 2;
                            g.DrawImage(transparent, new Rectangle(dx2, dy2, dw2, dh2), hoverBounds, GraphicsUnit.Pixel);
                        }

                        string outPath = Path.Combine(outputDir, fileNames[i]);
                        sheet.Save(outPath, ImageFormat.Png);
                        Console.WriteLine("Saved 2-frame animated sheet: " + outPath);
                    }
                }
            }
        }
    }
}
